...
## Gráfico de árbol (Treeplot)

- El gráfico de árbol es una forma de visualizar la jerarquía de categorías en un conjunto de datos

- Se utiliza para mostrar la estructura de categorías y subcategorías

- En R, se puede utilizar la librería `treemap` para crear un gráfico de árbol

- Se necesita instalar la librería con `install.packages("treemap")`

## Empresas por provincia y sector (CIIU)

```{r}
#| label: grafico-arbol-provincia-sector
#| eval: false
...
supercias_filtrado %>%
  group_by(provincia, ciiu_nivel_1) %>%
  summarise(frecuencia = n()) %>%
  treemap(index = c("provincia", "ciiu_nivel_1"), vSize = "frecuencia", vColor = "frecuencia", type = "index") +
  labs(title = "Empresas por Provincia y Sector (CIIU)")
```
...
```{r}
#| label: grafico-arbol-provincia-sector-grafico
#| echo: false
